...
public abstract partial class SharedRMCTelephoneSystem : EntitySystem
{
    ...
    {
        // Simply stop ringing and hang up on any attack
        StopSound(ent);
        _audio.PlayPredicted(RemoteHangupSound, ent, args.Attacker);
        _popup.PopupClient(Loc.GetString("rmc-phone-attack-shutdown", ("phone", ent)), args.Attacker, args.Attacker);
    }
    ...
    private void OnPickedUpPhoneGettingInsertedAttempt(Entity<RMCPickedUpPhoneComponent> ent, ref ContainerGettingInsertedAttemptEvent args)
    {
        ...
        if (ent.Comp.OriginalHolder is { } holder &&
            ...
            _hands.TryGetHand(holder, args.Container.ID, out _))
        {
            return;
        }
    ...
    private void OnTelephoneCallMsg(Entity<RotaryPhoneComponent> ent, ref RMCTelephoneCallBuiMsg args)
    {
        ...
        {
            _popup.PopupEntity(Loc.GetString("rmc-phone-busy"), user, user, PopupType.MediumCaution);
            return;
        }
        ...
        {
            _popup.PopupEntity(Loc.GetString("rmc-phone-no-transmitter"), user, user, PopupType.MediumCaution);
            return;
        }

        // Popup for the caller (no MarineComponent check)
        if (TryComp(user, out MetaDataComponent? userMeta) &&
            TryComp(ent, out MetaDataComponent? phoneMeta))
        {
            _popup.PopupEntity(Loc.GetString("rmc-phone-dialing", ("caller", userMeta.EntityName), ("phone", phoneMeta.EntityName)), ent);
        }
    ...
    protected bool TryGetPhoneHandHolder(EntityUid phone, out EntityUid holder)
    {
        holder = default;
        ...
        while (pickedUpPhonesQuery.MoveNext(out var uid, out var pickedUp, out var telephone))
        {
            ...
            if (ShouldReturnPickedUpPhone(uid, pickedUp, rotary))
            {
                if (ReturnPickedUpPhone(rotary, uid, null))
                    _popup.PopupEntity(Loc.GetString("rmc-phone-snap-back", ("phone", Name(uid)), ("rotary", Name(rotary))), uid, PopupType.MediumCaution);
